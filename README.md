...
Если указано несколько ключей при set или delete - будет возвращено 405 \
Если будет указан несуществующей ключ при get - будет возвращено 404
При любом ином запросе - будет возвращено 403
...
Скрипт предварительно создает директорию certs, где будут находиться ключи, сертификаты \
Запуск скрипта:
```
